...
function showItem(){
    ...
    for(i=0; i<backpack.length; i++){
        document.getElementById('backpack').innerHTML +=
        '<li><div id="item'+i+'">'+backpack[i]+'</div>  <button onclick="editItem('+i+')">E</button>  <button onclick="removeItem('+i+')">X</button>  <button onclick="upItem('+i+')">ᐱ</button>  <button onclick="downItem('+i+')">ᐯ</button></li>'
    }
    n = document.getElementById('b').innerHTML = backpack.length+'/'+size+' ITEMS - <button onclick="resetList()">CLEAR LIST</button>'
}
